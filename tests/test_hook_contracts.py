"""Hook contract tests for deterministic Claude plugin execution paths."""
...
def _hook_config(repo_root: Path, event_name: str, index: int = 0) -> dict:
...
def _run_hook(
    ...
    *,
    ...
    hook = _hook_config(repo_root, event_name, index=index)
    command = hook["command"].replace("${CLAUDE_PLUGIN_ROOT}", str(repo_root))
...
def _bootstrap_session(repo_root: Path, env: dict[str, str], project_root: Path, session_id: str) -> Path:
    ...
    project_dir = Path(env["MEGA_CODE_DATA_DIR"]) / "projects" / get_project_folder_name(str(project_root))
...
def test_session_start_hook_bootstraps_data_and_session(repo_root: Path, test_env: dict[str, str], tmp_path: Path):
    ...
    assert (data_dir / "plugin-root").read_text(encoding="utf-8").strip() == str(repo_root)
    ...
    assert "run --directory" in Path(test_env["MEGA_CODE_TEST_UV_LOG"]).read_text(encoding="utf-8")
...
def test_user_prompt_submit_hooks_increment_stats_and_emit_pending_notice(
    ...
    pending_skill_dir = Path(test_env["MEGA_CODE_DATA_DIR"]) / "data" / "pending-skills" / "auth-refresh"
    ...
    pending_strategy = Path(test_env["MEGA_CODE_DATA_DIR"]) / "data" / "pending-strategies" / "auth-refresh.md"
...
def test_session_end_hook_finalizes_and_uploads_trajectory(
    ...
    store = LocalStore(db_path=Path(test_env["MEGA_CODE_DATA_DIR"]) / "local-runtime.sqlite3")
