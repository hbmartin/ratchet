"""Async pipeline runner for /ratchet:wisdom-gen command.
...
def _resolve_include_flags(
    ...
    *,
    ...
    if project_mode and not explicit_source_selection:
        config = load_config()
        include_claude = source_enabled("claude", config)
        include_codex = source_enabled("codex", config)
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run Ratchet pipeline and save to pending folders",
        epilog="""
Project argument formats:
  @ratchet           Name prefix (with @ for Claude Code autocomplete)
  ratchet            Name prefix (without @)
  ratchet_b39e0992   Exact folder name
...
""",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Also reads RATCHET_POLL_TIMEOUT env var."
        ),
    )
    parser.add_argument(
        "--poll-existing",
...
async def main():
    ...
    storage = args.storage or os.environ.get("RATCHET_PIPELINE_STORAGE", "local")
    ...
    with tracer.start_as_current_span("run_pipeline") as span:
        ...
        try:
            ...
            if args.project:
                ratchet_project_dir = resolve_project_path(args.project)
            else:
                ratchet_project_dir = get_project_sessions_dir(str(project_dir_env))

            project_id = ratchet_project_dir.name
            span.set_attribute("pipeline.project_dir", str(ratchet_project_dir))
            ...
            if args.session_id:
                ...
                resolved_project_path = ratchet_project_dir
            ...
            if args.poll_timeout is not None:
                ...
                env_val = os.environ.get("RATCHET_POLL_TIMEOUT", "1200")
                try:
                    _raw = int(env_val)
                except ValueError:
                    logger.warning(
                        "Invalid RATCHET_POLL_TIMEOUT=%r (must be integer); using default 1200",
                        env_val,
                    )
                    _raw = 1200
            ...
            # Check for local poll timeout — exit code 3 tells the run skill
            ...
            if status.status == "timeout":
                timeout_info = {
                    "additionalContext": (
                        f"The local pipeline timed out ({status.error}).\n"
                        f"Inspect it with: ratchet pipeline-inspect --run-id {run_id}\n"
                        f"Tail logs with: ratchet pipeline-logs --run-id {run_id} --tail 160"
                    ),
                    "timeout": {
                        "run_id": run_id,
                        "project_id": project_id,
            ...
            if status.status == "failed":
                ...
                failed_info = {
                    "additionalContext": (
                        format_error_notification(error_msg).strip()
                        + "\n\n"
                        + f"Inspect it with: ratchet pipeline-inspect --run-id {run_id}\n"
                        + f"Tail logs with: ratchet pipeline-logs --run-id {run_id} --tail 160\n"
                        + f"Bundle evidence with: ratchet debug-bundle --run-id {run_id}"
                    ),
                    "failed": {
                        "run_id": run_id,
                        "project_id": project_id,
