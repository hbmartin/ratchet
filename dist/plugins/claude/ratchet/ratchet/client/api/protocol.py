"""Client protocol and response models for the Ratchet API.
...
class PendingLessonData(BaseModel):
    ...
    IMPORTANT: Mirrored in ratchet.server.models.PendingLessonData.
...
class RatchetBaseClient(Protocol):
    ...
    Implementations:
    - RatchetRemote: HTTP client to a Ratchet FastAPI server
    - RatchetLocal: In-process implementation for server-side use
    """
