...
- `ratchet.client.dirs.data_dir()` → data root path
- `ratchet.client.skill_installer.skills_dir()` → skills directory
- `ratchet.client.skill_installer.install_skills(skills)` → download + extract
- `ratchet.client.curation_store.save_curation(result)` → save to pending/
- `ratchet.client.curation_store.get_curation(session_id)` → load by ID
- `ratchet.client.curation_store.list_curations(status)` → list by status
- `ratchet.client.curation_store.update_curation_status(id, status)` → transition
