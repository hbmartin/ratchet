...
# Usage: launch-viewer.sh <RATCHET_DIR> <ITER_DIR> <SKILL_NAME> <ITERATION_NUM>
set -euo pipefail

if [ $# -lt 4 ]; then
    echo "Usage: $(basename "$0") <RATCHET_DIR> <ITER_DIR> <SKILL_NAME> <ITERATION_NUM>" >&2
    exit 1
fi
...
uv run --directory "$RATCHET_DIR" python -m ratchet.client.enhancement_viewer \
    "$ITER_DIR" \
