...
from ratchet.client.eval_workspace import save_artifact
...
def _classify_trust(frontmatter: dict, source: str = "auto") -> tuple[str, str, str]:
    ...
    Two trust levels:
    - ``trusted`` — the skill has a ratchet author marker.
    - ``semitrusted`` — the skill does not have a ratchet author marker.
    """
    if source == "same-repo":
        return (
            "trusted",
    ...
    if source == "unknown" or RATCHET_AUTHOR_MARKER not in author:
        return (
            "semitrusted",
            "unknown_author_or_source",
            "The skill does not have a ratchet author marker, so it is treated as semitrusted.",
        )

    return (
        "trusted",
        "ratchet_authored",
        "The skill has a ratchet author marker, so it is treated as trusted.",
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        prog="ratchet.client.skill_security_audit",
