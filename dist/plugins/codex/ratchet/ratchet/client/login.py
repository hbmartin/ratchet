"""CLI login flow for Ratchet.

Authenticates users via OAuth (GitHub/Google) through ratchet-service's CLI session
endpoints. Supports two modes:

**Combined mode** (default, for terminal use):
    python -m ratchet.client.login

**Two-step mode** (for Claude Code / non-interactive use):
    # Step 1: Create session, print JSON with login_url (fast, non-blocking)
    python -m ratchet.client.login --step create

    # Step 2: Poll until complete, save API key (run in background)
    python -m ratchet.client.login --step poll --client-id <ID> --url <URL>
"""
...
_DEFAULT_SERVER_URL = "https://console.ratchetai.ai"
# Keep the existing service wire path until the backend is renamed.
_RATCHET_SERVICE_API_PATH = "/api/ratchetai-service/v1"
...
def _resolve_ratchet_service_url() -> str:
    """Resolve the ratchet-service API URL from RATCHET_SERVER_URL.

    Reads the server base URL and appends the API path.

    Priority:
    1. RATCHET_SERVER_URL from the .env file
    2. RATCHET_SERVER_URL environment variable
    3. Default URL
    """
    env_vars = load_env_file(get_env_path())
    server = (
        env_vars.get("RATCHET_SERVER_URL")
        or os.environ.get("RATCHET_SERVER_URL")
        or _DEFAULT_SERVER_URL
    )
    return server.rstrip("/") + _RATCHET_SERVICE_API_PATH


def _derive_server_url(base_url: str) -> str:
    """Derive RATCHET_SERVER_URL from a ratchet-service API URL.

    Strips the current ratchet-service API suffix if present.
    """
    if base_url.endswith(_RATCHET_SERVICE_API_PATH):
        return base_url[: -len(_RATCHET_SERVICE_API_PATH)]
    return base_url


def create_cli_session(base_url: str, provider: str) -> dict:
    """Create a CLI auth session on ratchet-service.

    Args:
        base_url: ratchet-service API base URL.
    ...
    Raises:
        ...
        httpx.ConnectError: If ratchet-service is unreachable.
    """
    resp = httpx.post(
...
def _save_api_key(api_key: str, base_url: str) -> tuple[Path, dict[str, str]]:
    ...
    Sets:
      - RATCHET_API_KEY
      - RATCHET_CLIENT_MODE=remote (if not already set)
      - RATCHET_SERVER_URL (if not already set, derived from base_url)
    ...
    env_vars["RATCHET_API_KEY"] = api_key

    # Ensure remote client mode and server URL are configured
    if env_vars.get("RATCHET_CLIENT_MODE") != "remote":
        env_vars["RATCHET_CLIENT_MODE"] = "remote"

    # Always update server URL to match the service we authenticated against
    env_vars["RATCHET_SERVER_URL"] = _derive_server_url(base_url)
...
def run_create(
    ...
    """
    if base_url is None:
        base_url = _resolve_ratchet_service_url()

    try:
        session = create_cli_session(base_url, provider)
    except httpx.ConnectError:
        print(json.dumps({"error": f"Cannot connect to ratchet-service at {base_url}"}))
        return 1
...
def run_poll(client_id: str, base_url: str) -> int:
    ...
    try:
        ...
        print(f"Error: Lost connection to ratchet-service at {base_url}")
        return 1
...
def run_login(
    ...
    Args:
        provider: OAuth provider ("github" or "google").
        base_url: ratchet-service URL. Auto-resolved if None.
    ...
    try:
        ...
        print(f"Error: Cannot connect to ratchet-service at {base_url}")
        return 1
    except httpx.HTTPStatusError as exc:
    ...
    try:
        ...
        print(f"\nError: Lost connection to ratchet-service at {base_url}")
        return 1
    except (httpx.HTTPStatusError, ValueError) as exc:
        print(f"\nError: {exc}")
        return 1
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="ratchet-login",
        description="Sign in to Ratchet via OAuth",
    )
    parser.add_argument(
        "--step",
        choices=["create", "poll"],
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Server URL (e.g. https://console.ratchetai.ai). Appends API path automatically.",
    )
    parser.add_argument(
        "--client-id",
        type=str,
    ...
    if args.url and not args.url.rstrip("/").endswith(_RATCHET_SERVICE_API_PATH.rstrip("/")):
        args.url = args.url.rstrip("/") + _RATCHET_SERVICE_API_PATH
    ...
    if args.step == "create":
        ...
        if not args.client_id:
            ...
            return 1
        base_url = args.url or _resolve_ratchet_service_url()
