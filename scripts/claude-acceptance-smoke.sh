...
Run a repo-local acceptance smoke test for the MEGA-Code plugin using real
Claude CLI invocations plus the local deterministic MEGA runtime.

Options:
  --model <name>          Claude model alias for the prompt sessions.
  --data-dir <path>       Reuse an existing MEGA data dir instead of mktemp.
...
extract_last_json_field() {
  local log_path=$1
  local field_name=$2

  python3 - "$log_path" "$field_name" <<'PY'
import json
import sys

log_path, field_name = sys.argv[1:3]
...
set -a
. "$MEGA_DIR/.env" 2>/dev/null || true
set +a

if [[ -z "$DATA_DIR" ]]; then
  DATA_DIR="$(mktemp -d /tmp/mega-code-acceptance-data.XXXXXX)"
  TEMP_DATA_DIR=1
fi

if [[ -z "$FIXTURE_DIR" ]]; then
  FIXTURE_DIR="$(mktemp -d /tmp/mega-code-acceptance-fixture.XXXXXX)"
  TEMP_FIXTURE_DIR=1
fi

ARTIFACT_DIR=${ARTIFACT_DIR:-$(mktemp -d /tmp/mega-code-acceptance-artifacts.XXXXXX)}
...
[project]
name = "mega-code-acceptance-fixture"
version = "0.1.0"
EOF
...
rg -q 'MEGA-Code Status' "$STATUS_LOG" || fail "Status smoke did not return the status summary."
...
run_local \
  "$FIXTURE_DIR" \
  "$PIPELINE_LOG" \
  uv run --directory "$REPO_ROOT" python -m mega_code.client.run_pipeline --project --poll-timeout 60
...
run_local \
  "$FIXTURE_DIR" \
  "$REVIEW_LOG" \
  uv run --directory "$REPO_ROOT" python -m mega_code.client.pending review --run-id "$RUN_ID" --project-id "$PROJECT_ID"

rg -q 'MANDATORY WORKFLOW FOR CLAUDE' "$REVIEW_LOG" || fail "Pending review output was not rendered."

summary "MEGA-Code Claude acceptance smoke test"
summary "Repo root: <repo-root>"
summary "Claude auth mode: detected and usable"
summary "MEGA local model mode: fake via MEGA_CODE_TEST_FAKE_LLM=1"
summary "Fixture project: auth refresh sample"
summary "Status smoke:"
summary "PASS - slash command /mega-code:status loaded from --plugin-dir"
