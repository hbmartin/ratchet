...
def _chunks(items: list[str], size: int = SQLITE_IN_CHUNK_SIZE) -> Iterator[list[str]]:
...
class LocalStore:
    ...
    def _init_schema(self) -> None:
        with self._connect() as conn:
            ...
            self._ensure_column(conn, "pipeline_runs", "run_dir", "TEXT")
            self._ensure_column(conn, "pipeline_runs", "log_path", "TEXT")
            self._ensure_column(conn, "pipeline_runs", "events_path", "TEXT")
            self._ensure_column(conn, "pipeline_runs", "last_heartbeat_at", "TEXT")
            self._ensure_column(
                conn,
                "pipeline_runs",
                "heartbeat_count",
            ...
            conn.executescript(
                ...
                CREATE INDEX IF NOT EXISTS idx_pipeline_events_run_seq
                    ON pipeline_events(run_id, event_seq);
                CREATE INDEX IF NOT EXISTS idx_pipeline_llm_calls_run_seq
                    ON pipeline_llm_calls(run_id, call_seq);
    ...
    def _default_run_paths(self, project_id: str, run_id: str) -> dict[str, str]:
        run_dir = data_dir() / "runs" / _safe_path_part(project_id) / _safe_path_part(run_id)
        return {
            "run_dir": str(run_dir),
    ...
    def _ensure_debug_files(self, paths: dict[str, str]) -> None:
        ...
        with self._connect() as conn:
            conn.execute(
                ...
                INSERT INTO pipeline_events(
                    event_id, run_id, created_at, level, phase,
            ...
            self._touch_heartbeat(conn, run_id, event["created_at"])
    ...
    def record_llm_call(
        ...
        *,
        ...
        """Persist full local LLM call artifacts and an index row for inspection."""
        paths = self.ensure_run_debug_paths(run_id)
        call_id = str(uuid.uuid4())
        safe_label = _safe_path_part(label)[:80]
        llm_dir = Path(paths["run_dir"]) / "llm"
        llm_dir.mkdir(parents=True, exist_ok=True)
        prefix = f"{datetime.now(UTC).strftime('%Y%m%dT%H%M%S%fZ')}-{safe_label}-{call_id[:8]}"
        prompt_path = ""
        response_path = ""
        if prompt_text:
            prompt_file = llm_dir / f"{prefix}-prompt.txt"
            prompt_file.write_text(prompt_text, encoding="utf-8")
            prompt_path = str(prompt_file)
        if response_text or error:
            response_file = llm_dir / f"{prefix}-response.txt"
            response_file.write_text(response_text or error, encoding="utf-8")
        ...
        with self._connect() as conn:
            conn.execute(
                ...
                INSERT INTO pipeline_llm_calls(
                    call_id, run_id, created_at, label, provider, model,
                    call_type, duration_ms, status, prompt_path, response_path,
                    error, payload_json
                )
                VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
            ...
            self._touch_heartbeat(conn, run_id, row["created_at"])
    ...
    def list_pipeline_events(self, run_id: str) -> list[dict[str, Any]]:
        with self._connect() as conn:
            rows = conn.execute(
                """
                SELECT event_seq, event_id, run_id, created_at, level, phase,
                       event_type, message, payload_json
    ...
    def list_pipeline_llm_calls(self, run_id: str) -> list[dict[str, Any]]:
        with self._connect() as conn:
            rows = conn.execute(
                """
                SELECT call_seq, call_id, run_id, created_at, label, provider,
                       model, call_type, duration_ms, status, prompt_path,
                       response_path, error, payload_json
    ...
    def get_pipeline_run_row(self, run_id: str) -> dict[str, Any]:
        with self._connect() as conn:
    ...
    def list_pipeline_runs(self, *, include_terminal: bool = True, limit: int = 20) -> list[PipelineStatusResult]:
        ...
        results: list[PipelineStatusResult] = []
        for row in rows:
            row = self._reconcile_row(row)
            results.append(self._status_from_row(row))
        return results
    ...
    def create_run(
        ...
        *,
        ...
        paths = self._default_run_paths(project_id, run_id)
        self._ensure_debug_files(paths)
        ...
        with self._connect() as conn:
            conn.execute(
                ...
                INSERT OR REPLACE INTO pipeline_runs(
                    ...
                    status, progress_json, run_dir, log_path, events_path,
                    last_heartbeat_at, heartbeat_count, trace_id, started_at,
                    requested_stop
                )
                VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)
                """,
        ...
        if row["status"] in ACTIVE_STATUSES and not self._is_pid_alive(row["pid"]):
            ...
            with self._connect() as conn:
                conn.execute(
                    ...
                    WHERE run_id=?
                    """,
                    (error, completed_at, completed_at, row["run_id"]),
                )
    ...
    def stop_run(self, run_id: str) -> tuple[int | None, PipelineStatusResult]:
        with self._connect() as conn:
            ...
            conn.execute(
                ...
                WHERE run_id=?
                """,
                (stopped_at, "Stopped by user.", stopped_at, run_id),
            )
        self.record_pipeline_event(
            run_id,
            event_type="stop_requested",
            message="Pipeline stop requested by user.",
            phase="stopped",
            payload={"pid": row["pid"]},
        )
    ...
    def _status_from_row(self, row: sqlite3.Row) -> PipelineStatusResult:
        ...
        *,
        ...
        with self._connect() as conn:
            ...
            existing_rows = conn.execute(
                """
                SELECT o.operator_id, o.normalized_intent, o.slot_signature,
                       f.fingerprint_hash
                FROM operators o
                LEFT JOIN operator_env_fingerprints f ON f.operator_id = o.operator_id
                WHERE o.project_id = ?
                """,
                (project_id,),
            ).fetchall()
            existing_records = [dict(row) for row in existing_rows]
            existing_by_signature_map: defaultdict[tuple[str, str], list[dict[str, Any]]] = defaultdict(list)
            existing_and_new_by_intent: defaultdict[str, list[dict[str, Any]]] = defaultdict(list)
            for row in existing_records:
                existing_by_signature_map[(row["normalized_intent"], row["slot_signature"])].append(row)
                existing_and_new_by_intent[row["normalized_intent"]].append(row)
            ...
            for payload in operator_payloads:
                record = {
                    ...
                    "slot_signature": payload["slot_signature"],
                    "fingerprint_hash": payload["env_fingerprint"]["fingerprint_hash"],
                }
                existing_and_new_by_intent[record["normalized_intent"]].append(record)
            for operator in operator_payloads:
                ...
                for previous in existing_by_signature_map[
                    ...
                    if previous["operator_id"] == operator["operator_id"]:
                        continue
                    self._insert_operator_edge(
                        conn,
                        source_operator_id=operator["operator_id"],
                ...
                for row in existing_and_new_by_intent[operator["normalized_intent"]]:
                    if row["operator_id"] == operator["operator_id"]:
                        continue
                    if row.get("fingerprint_hash") == new_fingerprint_hashes[operator["operator_id"]]:
                        continue
                    if operator["slot_signature"] != row["slot_signature"]:
                        self._insert_operator_edge(
                            conn,
                            source_operator_id=operator["operator_id"],
    ...
    def fetch_fragments(self) -> list[dict[str, Any]]:
        with self._connect() as conn:
            rows = conn.execute(
                ...
                LEFT JOIN artifacts a ON a.artifact_id = p.artifact_id
    ...
    def _summarize_operator_reliability(
        ...
        predicted_success_sum = float(payload.get("predicted_success_sum", 0.0))
        confidence_sum = float(payload.get("confidence_sum", 0.0))
        mean_predicted_success = (
            predicted_success_sum / outcome_count
            if outcome_count
            else 0.5
        )
        mean_predicted_confidence = (
            confidence_sum / outcome_count
            if outcome_count
            else 0.35
        )
        calibration_error = (
        ...
        if outcome_count:
            prior_success = clamp(0.8 * prior_success + 0.2 * mean_predicted_success)
        retrieval_precision = (
        ...
        confidence = clamp(
            ...
            + 0.2 * max(0.0, 1.0 - calibration_error)
            + 0.25 * retrieval_precision
            + 0.1 * mean_predicted_confidence,
            lower=0.05,
            upper=0.99,
        )
    ...
    def _fetch_index_rows(
        ...
        for chunk in _chunks(operator_ids):
