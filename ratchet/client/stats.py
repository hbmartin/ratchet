"""Statistics aggregator for Ratchet sessions (client edition).
...
from ratchet.client.schema import (
...
def get_project_folder_name(project_dir: str) -> str:
    ...
    Examples:
        /Users/foo/ratchet → ratchet_a1b2c3d4
...
def resolve_project_path(project_arg: str) -> Path:
    """Resolve a project argument to a ratchet data folder path.

    Supports three input formats:
    1. @prefix or name prefix: fuzzy match against mapping.json keys
       e.g. '@ratchet' or 'ratchet' -> ~/.local/ratchet/projects/ratchet_b39e0992/
    2. Folder name with hash: direct lookup
       e.g. 'ratchet_b39e0992' -> ~/.local/ratchet/projects/ratchet_b39e0992/
    3. Absolute/relative path: resolve via get_project_sessions_dir()
       e.g. '/Users/foo/my-project' -> ~/.local/ratchet/projects/my-project_a1b2c3d4/

    Args:
        project_arg: Project identifier (with optional @ prefix).

    Returns:
        Path to the ratchet project data folder.
    ...
    # Strategy 1: Exact folder name match (e.g. 'ratchet_b39e0992')
    candidate = projects_dir / arg
    if candidate.is_dir():
        _logger.info(f"Resolved project by exact folder name: {arg}")
        return candidate

    # Strategy 2: Prefix match against mapping keys (e.g. 'ratchet')
    matches = [
        folder_name
