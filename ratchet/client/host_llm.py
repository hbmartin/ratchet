...
Canonical location — previously lived in ``ratchet.pipeline.host_llm`` but
moved here because the module has zero pipeline dependencies and must be
available in the OSS distribution (which ships only ``ratchet.client``).
"""
...
_MAX_CONCURRENCY = int(os.getenv("RATCHET_EVAL_CONCURRENCY", "4"))
_EVAL_TIMEOUT_SECONDS = float(os.getenv("RATCHET_EVAL_TIMEOUT", "300"))
