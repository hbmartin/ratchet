...
class DataLoader:
    ...
    from different data sources (Claude native, Ratchet, Parquet datasets).
...
def create_loader(
    ...
    Args:
        ...
        include_ratchet: Include ~/.local/ratchet source.
        claude_native_path: Custom path for Claude native data.
        ratchet_path: Custom path for Ratchet data.
    ...
    if include_ratchet:
        _try_register(RatchetSource, "Ratchet", base_path=ratchet_path)
...
def load_session_by_id(session_id: str) -> Session:
    ...
    Tries RatchetSource first, then ClaudeNativeSource.
    ...
    """
    loader = DataLoader()
    loader.register_source(RatchetSource())
    loader.register_source(ClaudeNativeSource())

    # Try RatchetSource first, fall back to ClaudeNativeSource
    try:
        return loader.load_from("ratchet", session_id)
    except KeyError:
...
def load_sessions_from_project(
    ...
    Loads Ratchet sessions. Optionally enriches with related Claude native sessions
    and/or Codex CLI sessions when flags are enabled. Sessions are deduplicated by
    session_id (Ratchet takes precedence over Claude). Codex sessions are appended
    ...
    Returns:
        List of Session objects (Ratchet only, or enriched with Claude/Codex).
    """
    # Load Ratchet sessions
    source = RatchetSource()
    ratchet_sessions = list(source.iter_sessions_from_path(project_path))
    logger.info(f"Loaded {len(ratchet_sessions)} Ratchet sessions from {project_path}")

    # Early return if not including Claude or Codex sessions
    if not include_claude and not include_codex:
        result = ratchet_sessions[:limit] if limit else ratchet_sessions
        if limit:
    ...
    project_paths = set()
    for session in ratchet_sessions:
        if session.metadata.project_path:
            project_paths.add(session.metadata.project_path)

    if not project_paths:
        if not include_claude and not include_codex:
            return ratchet_sessions[:limit] if limit else ratchet_sessions
        # Fall back to the project_path argument for enrichment sources
        project_paths.add(str(project_path))
        logger.debug(f"No Ratchet sessions; using project_path argument: {project_path}")

    logger.debug(f"Found {len(project_paths)} unique project paths in Ratchet sessions")
    ...
    for session in ratchet_sessions:
        session_map[session.metadata.session_id] = session
        logger.debug(f"Loaded session {session.metadata.session_id} from ratchet")

    # Build session map for deduplication (Ratchet > Claude)
    # Merge: deduplicated sessions + appended Codex
    merged = list(session_map.values())
    merged.extend(codex_sessions)

    logger.info(
        f"Merged: {len(ratchet_sessions)} Ratchet + "
